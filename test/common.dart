...
void testCsv() {
  group('csv spreadsheet:', () {
    test('Check format', () {
      var csvContent = 'Name,Age,City\nJohn,30,NYC\nJane,25,LA';
      var decoder = TableParser.decodeCsv(csvContent);
      expect(decoder is CsvDecoder, isTrue);
      expect(decoder.mediaType, 'text/csv');
      expect(decoder.extension, '.csv');
    });
  ...
  group('xlsx spreadsheet update:', () {
    group('No update', () {
      test('Empty file', () {
        ...
        var decoder = TableParser.decodeBytes(data);
        expect(decoder.tables.length, 3);
        expect(decoder.tables['Sheet1']!.rows, []);
        expect(decoder.tables['Sheet2']!.rows, []);
        expect(decoder.tables['Sheet3']!.rows, []);
      ...
      test('dataUrl', () {
        ...
        var decoder =
            TableParser.decodeBytes(base64Decode(data.split(',').last));
        expect(decoder.tables.length, expectedTest.keys.length);
        decoder.tables.forEach((name, table) {
          expect(table.rows, expectedTest[name]);
        });
